...
function aiLungoOn(){AILUNGO++;AILAST=Date.now();
  const e=document.getElementById("aiSpin");if(e)e.classList.add("on");}
...
window.skelRegistra=skelRegistra;window.skelChiudiTutti=skelChiudiTutti;
  if(!AILUNGO&&!AIBUSY){const e=document.getElementById("aiSpin");if(e)e.classList.remove("on");}}
...
function aiBusy(){AIBUSY++;AILAST=Date.now();
  if(AIQUIET)return;
  const e=document.getElementById("aiSpin");if(e)e.classList.add("on");}
...
function aiDone(){AILAST=Date.now();AIBUSY=Math.max(0,AIBUSY-1);if(AILUNGO)return;if(AIBUSY===0){const e=document.getElementById("aiSpin");if(e)e.classList.remove("on");}}
...
const AZIONI_PESANTI=/svuotat|cancellat|eliminat|azzerat|ripulit|rimoss|sostituit|riscritt|generat|resettat/i;
function toast(msg){const e=document.getElementById("toast");if(!e)return;
  const testo=String(msg==null?"":msg);
  const annullabile=(typeof undoDisponibile==="function")&&undoDisponibile()
    &&(Date.now()-((typeof UNDO!=="undefined"&&UNDO.at)||0)<1500);
  if(annullabile){
    const pesante=AZIONI_PESANTI.test(testo);
    e.innerHTML=esc(testo)+' <button class="tundo" onclick="annullaUltima()">'+
      (typeof LANG!=="undefined"&&LANG==="en"?"Undo":"Annulla")+'</button>';
    e.classList.add("on");clearTimeout(toastT);
    toastT=setTimeout(()=>e.classList.remove("on"),pesante?15000:8000);
    return;}
  e.textContent=testo;e.classList.add("on");clearTimeout(toastT);toastT=setTimeout(()=>e.classList.remove("on"),2600);}
...
async function proxyCall(prompt,imgs,pilastro){
  ...
  try{
    ...
    if(r.status===422)throw new Error("blocked");
...
async function geminiCall(prompt,imgs,pilastro){
  ...
  const chiedi=(typeof LANG!=="undefined"&&LANG==="en")
    ? prompt+" IMPORTANT: write every human-readable text in ENGLISH. Keep the JSON structure, keys and field names exactly as requested."
    : prompt;
  ...
  for(const m of gemModels()){
    for(let attempt=0;attempt<3;attempt++){
      ...
      try{
        ...
        if(r.status===400){throw new Error("badkey");}                     // chiave/permessi
        if(!r.ok)throw new Error("HTTP "+r.status);
        const j=await r.json();
...
async function aiAsk(prompt,pilastro){return geminiCall(prompt,null,pilastro);}
async function aiAskVision(prompt,imgs,pilastro){return geminiCall(prompt,imgs,pilastro||"foto");}
...
function dietStr(){const D=S.diet;
  ...
  if(D.alcol&&!vietato("alcol"))L.push("alcol: "+D.alcol);
...
function rulesForPlan(){const r=rulesSnapshot();
  return " QUALITÀ NUTRIZIONALE (vincolante): "+nutriRulesShort()+digiunoForAI()+" "+physForAI()+
    (r.custom?(" Regole della persona: "+r.custom):"");}
function rulesForAI(){const r=rulesSnapshot();
  return " QUALITÀ NUTRIZIONALE (vincolante): "+nutriRules()+digiunoForAI()+" "+physForAI()+famForAI()+hungerForAI()+chronoForAI()+crashForAI()+
...
/* Quello che accende la fame quando fame non c'è: dichiarato dalla persona
   ...
   non a etichettarla. */
/* ── IL RACCONTO ─────────────────────────────────────────────────────
   La persona scrive di sé come le viene. L'app legge, compila quello che
   riconosce e — soprattutto — dice cosa MANCA, invece di far indovinare.
   Nessun campo viene inventato: se una cosa non è stata detta, resta vuota
   e si chiede. */
/* ── LE BOLLE DEL RACCONTO ───────────────────────────────────────────
   Chi scrive «di sé» a foglio bianco dice troppo poco: servono tracce.
   Le bolle non spariscono quando le copri — si SPENGONO, barrate: così
   vedi che l'app ti ha capito, invece di dover ricordare cosa hai detto.
   Il riconoscimento mentre scrivi è locale (nessuna chiamata AI a ogni
   lettera); l'AI arriva dopo e corregge. */
const RACC_DOMANDE=[
 {k:"eta",   q:"quanti anni hai",        re:/\b(1[2-9]|[2-9]\d)\s*anni\b|\b(19|20)\d\d\b|\b\d{1,2}\/\d{1,2}\/(19|20)\d\d\b/i},
 {k:"corpo", q:"altezza e peso",         re:/\b1[4-9]\d\s*(cm)?\b.{0,24}\b\d{2,3}\s*(kg|chili)?\b|\b\d{2,3}\s*kg\b/i},
 {k:"goal",  q:"dove vuoi arrivare",     re:/dimagr|perdere|calare|mantener|aument|massa|tonific|\barrivare a\b|\bkg\b.{0,20}\b(obiettivo|target)\b/i},
 {k:"gior",  q:"com'è la tua giornata",  re:/lavor|ufficio|turni|scrivania|seduto|in piedi|viagg|casa|studi|figli|mensa/i},
 {k:"sport", q:"che sport ti piacciono", re:/tennis|palestra|corsa|corro|nuoto|calcio|padel|bici|yoga|pilates|cammin|danza|basket|squash|crossfit|pesi|allen/i},
 {k:"cibo",  q:"cosa non mangi",         re:/intolleran|allerg|celiac|lattosio|glutine|vegetarian|vegan|non mangio|evito|schifo|odio il|non digerisc/i},
 {k:"cuci",  q:"quanto tempo per cucinare", re:/cucin|fornelli|minut|tempo di prepar|schiscet|porto il pranzo|mangio fuori/i},
 {k:"fame",  q:"quando mangi senza fame", re:/stress|noia|annoi|solitud|triste|rabbia|nervos|sera mangio|abbuff|spilucc|ansia/i},
];
function raccontoCoperte(testo){
  const t=String(testo||"");
  const set=new Set();
  RACC_DOMANDE.forEach(d=>{if(d.re.test(t))set.add(d.k);});
  /* ciò che l'AI ha già scritto nei campi conta come detto */
  const val=(id)=>{const e=document.getElementById(id);return e&&String(e.value||"").trim();};
  if(val("obDob"))set.add("eta");
  if(val("obH")&&val("obW"))set.add("corpo");
  if(val("obTargetW"))set.add("goal");
  if(S.train&&(S.train.ama||S.train.dove))set.add("sport");
  if(S.diet&&(S.diet.intol||S.diet.avoid))set.add("cibo");
  if(S.diet&&S.diet.cookTime)set.add("cuci");
  if(S.diet&&(S.diet.trigger||[]).length)set.add("fame");
  return set;}
/* ── SECONDO GIRO: CHIEDERE SOLO CIÒ CHE MANCA ───────────────────────
   Il primo giro mostra otto etichette fisse. Va bene per capire cosa
   serve, ma dopo che la persona ha raccontato è una domanda sprecata:
   ripete anche le cose che ha già detto. Qui si guarda cosa è rimasto
   scoperto e si chiede QUELLO, con parole legate al suo racconto —
   «hai scritto che giochi a tennis: dove ti alleni, palestra o circolo?»
   invece di «che sport ti piacciono».
   Senza AI il secondo giro esiste lo stesso, con le domande generiche:
   il valore è chiedere poco, non chiedere bene. */
/* ATTENZIONE AL NOME: esiste già un raccontoMancanti() che risponde a
   un'altra domanda (che cosa manca per far PARTIRE il piano). Questa dice
   quali DOMANDE del racconto sono rimaste scoperte: nomi diversi, perché
   due funzioni con lo stesso nome nello stesso file si annullano a vicenda
   in silenzio — è già successo. */
function raccontoScoperte(testo){
  const fatte=raccontoCoperte(testo);
  return RACC_DOMANDE.filter(d=>!fatte.has(d.k));}
window.raccontoScoperte=raccontoScoperte;

let RACC_G2=[];                       /* le domande del secondo giro */
window.raccontoGiro2=async(silenzioso)=>{
  const ta=document.getElementById("obStory");
  const testo=(ta&&ta.value||"").trim();
  const box=document.getElementById("obBolle2");
  const nota=document.getElementById("obBolle2Nota");
  if(!box)return;
  const manca=raccontoScoperte(testo);
  if(!manca.length){                  /* niente da chiedere: si sparisce */
    RACC_G2=[];box.innerHTML="";if(nota)nota.textContent="";return;}
  /* fallback immediato: le domande generiche, subito visibili */
  RACC_G2=manca.map(d=>({k:d.k,q:tr(d.q)}));
  raccontoGiro2Disegna();
  if(!aiOn()||!testo||silenzioso)return;
  try{
    const j=await aiAskJSON('Questa persona si è raccontata: """'+testo.slice(0,1500)+'""". '+
      'Mancano ancora queste informazioni: '+manca.map(d=>d.k+" ("+d.q+")").join(", ")+'. '+
      'Per OGNI voce mancante scrivi UNA domanda breve '+((typeof LANG!=="undefined"&&LANG==="en")?"in English":"in italiano")+', massimo 12 parole, '+
      'agganciata a ciò che ha già detto quando è possibile, mai una domanda già risposta. '+
      'Dai del tu, tono gentile, nessun giudizio. '+
      'Rispondi SOLO JSON: {"domande":[{"k":"chiave","q":"la domanda"}]}',"racconto2");
    const arr=(j&&Array.isArray(j.domande))?j.domande:[];
    if(!arr.length)return;
    /* si tengono solo le chiavi che mancano davvero: l'AI non decide cosa chiedere */
    const validi=new Set(manca.map(d=>d.k));
    const buone=arr.filter(x=>x&&validi.has(x.k)&&String(x.q||"").trim());
    if(!buone.length)return;
    RACC_G2=manca.map(d=>{
      const t=buone.find(x=>x.k===d.k);
      return {k:d.k,q:t?String(t.q).slice(0,120):tr(d.q)};});
    raccontoGiro2Disegna();
  }catch(e){}                          /* l'AI è un miglioramento, non un requisito */
};
function raccontoGiro2Disegna(){
  const box=document.getElementById("obBolle2");
  const nota=document.getElementById("obBolle2Nota");
  if(!box)return;
  box.innerHTML=RACC_G2.map((d,i)=>
    `<button type="button" class="rbolla g2" onclick="raccontoRispondi(${i})">${esc(d.q)}</button>`).join("");
  if(nota)nota.textContent=RACC_G2.length
    ? tr("Tocca una domanda per rispondere: quello che scrivi si aggiunge al racconto.")
    : "";}
/* Rispondere a una domanda: la risposta finisce nel racconto, non in un
   campo nascosto. Così resta leggibile e l'AI la rilegge insieme al resto. */
window.raccontoRispondi=async(i)=>{
  const d=RACC_G2[i];if(!d)return;
  const r=await dlgPrompt(d.q,"");
  if(r===null||!String(r).trim())return;
  const ta=document.getElementById("obStory");
  if(!ta)return;
  ta.value=(ta.value?ta.value.trim()+" ":"")+String(r).trim();
  try{S.profile.story=ta.value.slice(0,2000);save();}catch(e){}
  try{raccontoBolle();}catch(e){}
  raccontoGiro2(true);                /* la domanda risposta sparisce dal giro */
  toast(tr("Aggiunto al racconto ✓"));};

/* ── DETTARE INVECE DI SCRIVERE ──────────────────────────────────────
   Raccontarsi a voce viene molto più naturale che digitare, e il racconto
   esce più lungo e più sincero. Dove il browser espone il riconoscimento
   vocale (Chrome su Android usa quello di Google) mettiamo il pulsante;
   dove non c'è — iOS, quasi sempre — non fingiamo: si dice di usare il
   microfono della tastiera, che c'è su tutti i telefoni. */
...
window.voceIn=(campo,pulsante)=>{
  const ta=document.getElementById(campo||"obStory");
  const btn=document.getElementById(pulsante||"obMic");
  if(!ta)return;
  if(!vocePossibile())
  ...
  r.onresult=(ev)=>{
    ...
    ta.value=base+testo;
    if(ta.id==="obStory"){try{raccontoBolle();}catch(e){}}
    else{try{ta.dispatchEvent(new Event("input",{bubbles:true}));}catch(e){}}};
...
window.raccontoVoce=()=>voceIn("obStory","obMic");
window.raccontoBolle=()=>{
  const el=document.getElementById("obStory");
  const box=document.getElementById("obBolle");
  if(!box)return;
  const fatte=raccontoCoperte(el&&el.value);
  box.innerHTML=RACC_DOMANDE.map(d=>{
    const on=fatte.has(d.k);
    return `<span class="rbolla ${on?"fatta":""}">${on?"✓ ":""}${esc(tr(d.q))}</span>`;}).join("");
  const rest=RACC_DOMANDE.length-fatte.size;
  const nota=document.getElementById("obBolleNota");
  if(nota)nota.textContent=rest?tr("Restano {n} cose da dire — scrivile come ti vengono, anche in disordine.",{n:rest})
                              :tr("Hai detto tutto quello che mi serve. Se vuoi aggiungere altro, fai pure.");};
window.raccontoLeggi=async()=>{
  const el=document.getElementById("obStory");
  const testo=(el&&el.value||"").trim();
  const box=document.getElementById("obStoryOut");
  if(!testo){if(box){box.style.display="block";box.textContent=tr("Scrivi due righe su di te, anche disordinate: al resto penso io.");}return;}
  try{S.profile.story=testo.slice(0,2000);save();}catch(e){}
  if(!aiOn()){if(box){box.style.display="block";box.textContent=tr("Senza chiave AI non posso leggere il racconto: compila i campi qui sotto, ci vuole un minuto.");}return;}
  box.style.display="block";genBoxMostra(box);box.textContent=tr("Leggo…");
  try{
    const j=await aiAskJSON('Questa persona si racconta: """'+testo+'""". '+
      'Estrai SOLO ciò che dice davvero, senza dedurre e senza inventare. Campi assenti = null. '+
      'Rispondi SOLO JSON: {"nome":null,"sesso":"m|f|null","nascita":"GG/MM/AAAA|null","eta":null,"altezza":null,"peso":null,'+
      '"pesoObiettivo":null,"attivita":"sedentaria|leggera|media|alta|null","obiettivo":"dimagrire|mantenere|aumentare|null",'+
      '"intolleranze":null,"evita":null,"protocolli":null,"pastiAlGiorno":null,"tempoCucina":null,'+
      '"sportAmati":null,"sportSgraditi":null,"doveTiAlleni":null,"quandoTiAlleni":null,'+
      '"momentiSenzaFame":null,"note":null}',"racconto");
    if(!j)throw new Error("Non sono riuscito a leggere il racconto");
    const messo=raccontoApplica(j);
    try{raccontoBolle();}catch(e){}
    /* Letto il racconto, si chiede SOLO quello che è rimasto fuori. */
    try{raccontoGiro2();}catch(e){}
    const manca=raccontoMancanti();
    let t=messo.length?(tr("Ho compilato:")+" "+messo.join(", ")+"."):tr("Non ho trovato dati sicuri da compilare.");
    if(manca.length)t+="\n\n"+tr("Mi manca ancora:")+" "+manca.join(", ")+". "+tr("Puoi scriverlo qui sopra e rileggere, oppure compilare i campi.");
    else t+="\n\n"+tr("Non manca nulla di essenziale. Se vuoi aggiungere altro, scrivilo pure: più cose so, meglio ti seguo.");
    box.textContent=t;
    if(typeof render==="function")setTimeout(()=>{try{const e=document.getElementById("obStoryOut");if(e)e.textContent=t;}catch(_){}},50);
  }catch(e){box.textContent="";aiFail(e);}};

/* Scrive nei campi del percorso solo ciò che è stato detto davvero. */
function raccontoApplica(j){
  const messo=[];
  const set=(id,val,eti)=>{const e=document.getElementById(id);
    if(e&&val!=null&&String(val).trim()!==""){e.value=String(val);messo.push(eti);}};
  set("obName",j.nome,tr("nome"));
  if(j.sesso){const e=document.getElementById("obGen");if(e){e.value=(j.sesso==="f")?"f":"m";messo.push(tr("sesso"));}}
  set("obDob",j.nascita,tr("data di nascita"));
  set("obH",j.altezza,tr("altezza"));
  set("obW",j.peso,tr("peso"));
  set("obTargetW",j.pesoObiettivo,tr("peso obiettivo"));
  /* Ciò che serve più avanti si mette da parte, non si perde. */
  S.profile=S.profile||{};S.diet=S.diet||{};S.train=S.train||{};
  if(j.intolleranze)S.diet.intol=String(j.intolleranze);
  if(j.evita)S.diet.avoid=String(j.evita);
  if(j.tempoCucina)S.diet.cookTime=String(j.tempoCucina);
  if(j.sportAmati)S.train.ama=String(j.sportAmati);
  if(j.sportSgraditi)S.train.odia=String(j.sportSgraditi);
  if(j.doveTiAlleni)S.train.dove=String(j.doveTiAlleni);
  if(j.quandoTiAlleni)S.train.quando=String(j.quandoTiAlleni);
  if(j.momentiSenzaFame){
    const t=String(j.momentiSenzaFame).toLowerCase();
    S.diet.trigger=["stress","stanchezza","noia","solitudine","rabbia","tristezza","abitudine serale"].filter(k=>t.includes(k.split(" ")[0]));}
  if(j.note)S.profile.storyNote=String(j.note).slice(0,400);
  if(j.sportAmati||j.doveTiAlleni)messo.push(tr("allenamento"));
  if(j.intolleranze||j.evita)messo.push(tr("cosa non mangi"));
  save();
  return messo;}
...
/* Cosa manca DAVVERO per far partire il piano: si chiede solo questo. */
function raccontoMancanti(){
  const v=(id)=>{const e=document.getElementById(id);return e?String(e.value||"").trim():"";};
  const m=[];
  if(!v("obName"))m.push(tr("come ti chiamo"));
  if(!v("obDob"))m.push(tr("quando sei nato"));
  if(!v("obH"))m.push(tr("altezza"));
  if(!v("obW"))m.push(tr("peso"));
  if(!(S.train&&(S.train.ama||S.train.dove)))m.push(tr("che sport ti piacciono"));
  if(!(S.diet&&S.diet.intol)&&!(S.diet&&S.diet.avoid))m.push(tr("intolleranze o cibi che eviti"));
  return m;}
...
async function aiAskJSON(prompt,tag){
  let t=await aiAsk(prompt),j=null;
  try{j=parseAIJSON(t);}catch(e){}
  aiHealth(tag||"json",j!=null);
  if(j==null){
    t=await aiAsk(prompt+" ATTENZIONE: la risposta precedente non era JSON valido. Reinvia SOLO il JSON richiesto, senza alcun testo prima o dopo.");
    j=parseAIJSON(t);}
  return j;}
...
function aiReason(e){const m=String(e&&e.message||e);
  return {timeout:"tempo scaduto",rete:"rete caduta",quota:"limite Gemini raggiunto",busy:"server occupati",
      ...
      const r=await fetch("https://generativelanguage.googleapis.com/v1beta/models/"+m+":generateContent?key="+encodeURIComponent(key),{
        ...
        body:JSON.stringify({contents:[{parts:[{text:'Rispondi SOLO con questo JSON: {"ok":1}'}]}],
          generationConfig:{maxOutputTokens:512,responseMimeType:"application/json",
            /* i modelli che "pensano" (3.7+) consumano token nel ragionamento:
               con 64 finivano in MAX_TOKENS senza produrre testo. Qui il
               pensiero si spegne e il tetto basta comunque. */
            thinkingConfig:{thinkingBudget:0}}})});
      const ms=Date.now()-t0;
      if(!r.ok){
        let det="";try{const j=await r.json();det=(j.error&&j.error.message)||"";}catch(_){}
        righe.push("✗ <b>"+esc(m)+"</b> — HTTP "+r.status+(det?": "+esc(det.slice(0,90)):""));
        continue;}
      const j=await r.json();
      const c=(j.candidates&&j.candidates[0])||null;
      const txt=((c&&c.content&&c.content.parts)||[]).map(p=>p.text||"").join("").trim();
      righe.push((txt?"✓":"✗")+" <b>"+esc(m)+"</b> — "+(txt?("risponde in "+ms+" ms"):("nessun testo, finishReason: "+esc(String(c&&c.finishReason)))));
    }catch(e){righe.push("✗ <b>"+esc(m)+"</b> — "+esc(aiReason(e)));}
  }
