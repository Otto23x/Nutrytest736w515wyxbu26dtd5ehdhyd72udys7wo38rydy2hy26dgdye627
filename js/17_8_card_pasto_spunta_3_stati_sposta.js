...
function mealCard(pdi,mi){
  ...
  let h=`<div class="${cls}" data-meal="${pdi}-${mi}">
    ...
    <div class="mbody">
      <div class="mtop"><span class="mname tap" onclick="editMeal(${pdi},${mi})" title="${tr("Tocca per modificare")}">${esc(dispName)}${st.skip?" (saltato)":""}</span>${
        st.done&&!st.skip
      ...
      <div class="mdesc tap" onclick="editMeal(${pdi},${mi})" title="${tr("Tocca per modificare")}">${esc(cap(o.d))}</div>
  ...
  h+=`</select></div><div class="mtools filaicone" style="margin-top:8px">
    <button class="ibtn" title="${tr("Inventami un'alternativa con gli stessi macro")}" onclick="altMeal(${pdi},${mi})">${ic("dice",17)}</button>
    <button class="ibtn" title="${tr("Non ho un ingrediente: 3 sostituzioni AI")}" onclick="subIngr(${pdi},${mi})">${ic("swap",17)}</button>
    <button class="ibtn" title="${tr("Scatta la foto del piatto adesso")}" onclick="mealPhoto(${pdi},${mi})">${ic("camera",17)}</button>
      <button class="ibtn" title="${tr("Scegli una foto già salvata nella galleria")}" onclick="mealPhoto(${pdi},${mi},true)">${ic("gallery",17)}</button>
    <button class="ibtn" title="${tr("Sono ospite: registra il pasto con una stima")}" onclick="stealthOpen(${pdi},${mi})">${ic("guest",17)}</button>
    <button class="ibtn" title="${tr("Barcode: scansiona i prodotti di questo pasto (anche più di uno)")}" onclick="scanStart(${viewIdx()},${pdi},${mi})"><svg width="16" height="14" viewBox="0 0 24 20" style="vertical-align:-2px"><g fill="currentColor"><rect x="1" y="2" width="2" height="16"/><rect x="5" y="2" width="1" height="16"/><rect x="8" y="2" width="3" height="16"/><rect x="13" y="2" width="1" height="16"/><rect x="16" y="2" width="2" height="16"/><rect x="20" y="2" width="1" height="16"/><rect x="22" y="2" width="1.5" height="16"/></g></svg></button>`;
...
/* La sfumatura sul bordo destro è un'informazione, non una decorazione:
   compare solo quando ci sono davvero icone oltre il bordo. */
window.filaIconeSegna=()=>{
  document.querySelectorAll(".mtools.filaicone").forEach(f=>{
    try{f.classList.toggle("scorre",f.scrollWidth>f.clientWidth+2);}catch(e){}});};
...
window.mealPhoto=async(pdi,mi,gal)=>{
  ...
  try{const ph=await anyPhoto(gal,false);
    ...
    const nk=Math.round(j.kcal),np=Math.round(j.prot),nc=Math.round(j.carb)||0,nf=Math.round(j.gras)||0,nfib=Math.round(j.fibre)||0,nz=Math.round(j.zuccheri)||0;
...
/* ═══  SENSORE DI ABBIOCCO ═══════════════════════════════════════ */
const CRASH_SLOTS=[["mattina","Mattina"],["pomeriggio","Pomeriggio"],["sera","Sera"]];
...
function crashHTML(di){
  ...
  return `<div style="border-top:1px solid var(--linea);margin-top:16px;padding-top:14px">
    ...
    <div class="statolist">`+CRASH_SLOTS.map(([k,l])=>{
      const on=set.indexOf(k)>-1;
      return `<label class="stato ${on?"on":""}">
        <input type="checkbox" ${on?"checked":""} onchange="crashTog(${di},'${k}')">
        <span class="sl"><b>${l}</b><small>${{mattina:"Sonnolenza o fame dopo colazione",pomeriggio:tr("Il classico crollo del primo pomeriggio"),sera:"Stanchezza o voglia di dolce dopo cena"}[k]||""}</small></span></label>`;}).join("")+
...
function prepUntilOpts(di){
  ...
  for(let k=start;k<Math.min(7,start+3);k++){if(!PLAN[k])continue;
    o+=`<option value="${k}">fino a ${giorno(PLAN[k].day)}${k===di?" (oggi)":""} a cena</option>`;}
  return o;}
...
window.cycleToggle=async()=>{
  ...
  if(!await dlgConfirm(tr("Attivo la fase luteale?\n\nPer i prossimi {a} giorni il fabbisogno sale di circa il {b}% del metabolismo basale (~{c} kcal al giorno), che si SOMMANO al target.\n\nSi spegne da sola alla scadenza. In questi giorni la bilancia può salire di 1-2 kg per la ritenzione idrica: è acqua, non grasso.{d}",{a:cycleDaysMax(),b:cyclePct(),c:Math.round(bmr()*cyclePct()/100),d:PHYS_NEUTRAL_WARN})))
    {render(cur);return;}
  S.phys.cycleOn=true;S.phys.cycleStart=iso(new Date());stampPhys();save();render(cur);
