...
function mealCardStatic(pdi,mi){const m=PLAN[pdi].meals[mi];const o=templateOpt(pdi,mi);
  ...
  let h=`<div class="meal"><div class="mbody">
    ...
    <div class="mdesc tap" onclick="editMealPerm(${pdi},${mi})" title="${tr("Tocca per modificare")}">${esc(cap(o.d))}</div>
...
function renderPiano(){const el=document.getElementById("pg-piano");const ti=wd(new Date());
  ...
  PLAN.forEach((d,di)=>{
    h+=`<div class="dayname">${giorno(d.day)}${di===ti?'<span class="todaytag">OGGI</span>':""}</div><div class="dayctx">${d.ctx}</div>`;
    (d.meals||[]).forEach((m,mi)=>h+=mealCardStatic(di,mi));
    const pl=plannedTemplateOfDay(di);
    h+=`<div class="daytotal">${tr("Piano:")} <span>~${pl.k} kcal · ${pl.p}g proteine · ${pl.c}g carboidrati · ${pl.f}g grassi · ${pl.fib}g fibre · ${pl.z||0}g zuccheri</span></div>`;
  });
  el.innerHTML=h;}
...
function puntoSaluto(){
  ...
  const s=o<5?"Notte fonda":o<12?"Buongiorno":o<18?"Buon pomeriggio":"Buonasera";
  return s+(n?", "+esc(n):"");}
...
function renderPunto(){const el=document.getElementById("pg-punto");const di=viewIdx();
    ...
    <div class="glist">
      ...
      ${(!densMin("full")||(!ev&&(S.ui.evNo||{})[_vd]))?"":(ev?`<div class="grow on fatta">
        ...
        <div class="gl"><b>${tr("Evento del giorno")}</b>
          <small>Compleanno, cena fuori, giornata no…</small></div>
  ...
  h+=(function(){
      ...
      else if(!eat.k&&dopo.length){cap="Giornata da iniziare.";}
      ...
      const sotto=[];
      if(dopo.length)sotto.push(dopo.length===1?"Resta un pasto":"Restano "+dopo.length+" pasti");
  ...
  ${(function(){
    ...
    g+=`<div class="statolist" style="margin-top:12px">`
      ...
      +rigaLibera("phIll",illOn(),"Malattia","Deficit sospeso: si mangia a mantenimento","illSet(this.checked)")
      +rigaLibera("phDig",digiunoOn(),
          "        ".trim()+(digiunoOn()?DIGIUNI[digiunoTipo()].l:"Digiuno religioso"),
          digiunoOn()?DIGIUNI[digiunoTipo()].d:"Ramadan, Quaresima, digiuno totale: scegli quale",
...
function renderOggi(){const el=document.getElementById("pg-oggi");const di=viewIdx();
  ...
  (S.week.days[di].extras||[]).forEach((e,ei)=>{const done=e.st!=="skip";
    ...
    <div class="mdesc tap" onclick="editExtra(${di},${ei})" title="${tr("Tocca per modificare")}">${esc(cap(e.d))}</div><div class="mkcal">~${e.k} kcal · ${e.p||0}g proteine${e.c!=null?` · ${e.c}g carboidrati`:""}${e.f!=null?` · ${e.f}g grassi`:""}${e.fib!=null?` · ${e.fib}g fibre`:""}${e.z!=null?` · ${e.z}g zuccheri`:""}</div>
    <div class="mtools" style="margin-top:8px">
