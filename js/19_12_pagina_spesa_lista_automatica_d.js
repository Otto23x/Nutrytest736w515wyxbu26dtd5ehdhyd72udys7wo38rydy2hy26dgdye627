...
window.genShop=async(silent)=>{
  ...
  try{
    ...
    for(let att=0;att<2&&!arr;att++){
      try{
    const t=await aiAsk("Da questi pasti dei prossimi 7 giorni ("+shopWindowLabel()+") crea la lista della spesa SETTIMANALE"+dispensaForShop()+((!shopForMe()&&(S.family||[]).length)?" per "+(Math.round(famUnits()*100)/100)+" porzioni di riferimento (donna adulta=1, uomo=1,25, adolescente=1,10, bambino=0,75, infante=0,50)":" per UNA persona")+
    ...
    const pulita=o.filter(x=>Array.isArray(x)&&x.length>=2&&Array.isArray(x[1])&&x[1].length)
      .map(x=>[cap(String(x[0])),x[1].map(y=>prodottoPrima(String(y)))]);
    if(!pulita.length)throw new Error("formato non valido");
    arr=pulita;
...
function shopWindowLabel(){
  const t=wd(new Date());
  return PLAN[t]?("da "+PLAN[t].day+" a "+PLAN[(t+6)%7].day):"7 giorni";}
...
function renderSpesa(){const el=document.getElementById("pg-spesa");
  ...
  <div class="hint" style="margin-top:8px">${trh("Copre i {b} ({v1}). Si aggiorna da sola quando cambi il piano.",{b:"<b>7 giorni da oggi</b>",v1:esc(shopWindowLabel())})}</div></div>`;
  ...
  const rigaShop=(id,it,delFn,ci,ii)=>{const c=S.shop[id];   /* ci/ii: posizione in lista, serve alla sostituzione */
    return `<div class="shopitem ${c?"c":""}">
        ...
        <span class="st" onclick="tglShop('${id}')">${esc(prodottoPrima(it))}</span>
