...
function wizTargets(){
  ...
  const defMap={leggero:-300,moderato:-500,deciso:-750,mantenimento:0,massa:200};
  const kcal=tdeeV+(defMap[d.goal]??-500);
...
function deficitTarget(){
  ...
  if(/massa|aument/.test(g)){
    /* surplus coerente col ritmo scelto (kg presi a settimana), con tetto */
    const s=(defMode()==="ritmo")?Math.round(ratePerWeek()*7700/7):Math.round(t*0.12);
    return -Math.min(500,Math.max(150,s));}
  let d;
  if(defMode()==="pct")d=Math.round(t*defPct()/100);
  /* il ritmo scelto è il deficit, senza maggiorazioni nascoste: altrimenti
     l'app viaggiava a un ritmo diverso da quello dichiarato all'utente */
  else d=Math.round(ratePerWeek()*7700/7);
...
function outTypeHTML(pre,perSettimana){
  ...
  <div class="ckgrid">
    ...
    <label class="ck"><input type="radio" name="${pre}ot" ${c2!=="porto"?"checked":""} onchange="outTypeSet('fuori',${!!perSettimana})">  Li mangio fuori</label>
  </div>
...
function protKgAuto(){
  ...
  if(pregOn()!=="no"||lactKcal())return 1.7;
  const act=+S.profile.act||1.3;
  if(act<=1.25&&!goalWkTotal())return 1.2;
  return 1.5;}
function dayTargetP(){
...
function renderSetup(){const el=document.getElementById("pg-setup");let h="";
  ...
  if(WIZ.step===1){h=disc+`<div class="card"><h2>${tr("Passo 1 di 3 — Chi sei")}</h2>
    ...
    <select id="wzAct">${[["1.2","Molto sedentario"],["1.3","Sedentario, lavoro al PC"],["1.35","Poco attivo"],["1.4","Moderatamente attivo"],["1.45","Attivo"],["1.55","Molto attivo (lavoro fisico)"]].map(o=>`<option value="${o[0]}" ${String(WIZ.d.act||S.profile.act||1.3)===o[0]?"selected":""}>${o[1]}</option>`).join("")}</select>
    ...
    <label>Sport abituali</label><input type="text" id="wzSport" value="${esc(WIZ.d.sport||"")}" placeholder="${tr("es. camminata 4 km quasi ogni giorno, tennis saltuario")}">
...
NUVIA_IT.corpo=()=>`<div class="card nvhero">
  <img class="nvfloat" alt="Nuvia" src="${window.NUVIA_LOCKUP||''}" style="height:150px;width:auto" onerror="this.style.display='none'">
...
<tr><td colspan="2"><b>${tr("Codice piano")}</b> ${trh("(in Regole → Il piano, o nell'ultimo passo del benvenuto): carica un piano già pronto. {b} è la dieta standard con cui è nata Nuvia — mediterranea, cinque pasti, mensa il martedì e il giovedì.",{b:"<b>00000000</b>"})}</td></tr>
...
function renderIo(){const el=document.getElementById("pg-io");const p=S.profile;
  ...
  ${hint2("Lascia vuoto e l'obiettivo lo calcola l'app su di te.",
 `${trh("{v1}, già al netto dell'acqua che arriva dal cibo. Un bicchiere vale 200 ml; con allenamenti intensi o oltre 45 minuti l'obiettivo sale di 2 bicchieri.",{v1:waterExplain()})}`)}
  ...
  <option value="1.4" ${p.act==1.4?"selected":""}>Attivo</option>
  <option value="1.45" ${p.act==1.45?"selected":""}>Moderatamente attivo</option></select>
  ...
  h+=`<div class="card"><h2>Promemoria</h2>
...
function renderSistema(){const el=document.getElementById("pg-sistema");const p=S.profile;
  ...
  <div class="hint">${trh("Con la chiave gratuita usi i modelli {b}",{b:"<b>Flash</b>"})} ${trh("(auto = prova 3.5-flash → 3.1-flash-lite → 2.5). I {b} in fondo alla lista funzionano solo con fatturazione attiva: se li scegli senza, l'app ripiega da sola sui Flash.",{b:"<b>Pro</b>"})}</div>
 ...
 tr("Serve per: foto dei piatti e del frigo, lettura dei menù, stime di sfori e sport, alternative, ribilanciamento, sostituzioni nella spesa e report settimanali. Senza chiave l'app funziona lo stesso, ma tutto va fatto a mano."))}</div>`;
  ...
  h+=`<div class="card"><h2>${tr("Segnala un bug o un'idea")}</h2>
  <div class="hint">${tr("Hai trovato qualcosa che non funziona, o ti è venuta un'idea per migliorare Nuvia? Scrivila qui: si apre la tua email già pronta per lo sviluppatore.")}</div>
  <label>${tr("Di cosa si tratta")}</label>
  <select id="bugType">
    <option>${tr("Bug — qualcosa non funziona")}</option>
    <option>${tr("Miglioria — funziona ma si può fare meglio")}</option>
    <option>${tr("Nuova funzione — vorrei che facesse anche…")}</option>
    <option>${tr("Altro")}</option>
  </select>
  <label>Descrizione</label>
  <textarea id="bugTxt" rows="5" style="min-height:120px" placeholder="${tr("Cosa è successo, in quale schermata, cosa ti aspettavi. Se è un'idea, raccontala pure liberamente.")}"></textarea>
  <label class="ckline" style="display:flex;gap:8px;align-items:center;margin-top:8px">
    <input type="checkbox" id="bugCtx" checked style="width:20px;height:20px"> ${tr("Allega i dati tecnici: versione, dispositivo, dimensioni schermo")}</label>
  <div class="hint">${tr("Non vengono allegati i tuoi dati alimentari né il profilo: solo la versione dell'app e il tipo di dispositivo, per capire dove si è rotto.")}</div>
  <button class="btn ghost" onclick="sendBug()">${tr("Invia allo sviluppatore")}</button></div>`;
  // AI
  el.innerHTML=h;}
