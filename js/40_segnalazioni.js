...
const SEG_SCHERMI={
  punto:"Il punto", oggi:"La giornata", piano:"Il piano", spesa:"La spesa",
  ...
  benvenuto:"Benvenuto", onb2:"Primo avvio", piani:"Piani"
};
...
function segnalazioniHTML(){
  ...
  const sel=`<select id="segSchermo" onchange="segCambia()">
    ...
    ${opz.map(n=>`<option${n===b.schermo?" selected":""}>${esc(n)}</option>`).join("")}
  </select>`;
